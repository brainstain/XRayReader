...
from CapsNet import capslayer as cl
import model
...
class CapsNet:
    def __init__(self, input_shape, n_class, routings=3, kernal_size=9, conv_filters=256,
        ...
        decoder.add(layers.Dense(512, activation='relu', input_dim=digit_caps_dim * n_class))
        ...
        self.adam_optimizer = optimizers.Adam(lr=self.lr)
    ...
    def train(self, data, batch_size, steps_per_epoch, epochs=1):
        ...
        self.train_model.compile(optimizer=self.adam_optimizer,
    ...
    def test(self, data):
        ...
        img = combine_images(np.concatenate([input_images, x_recon[:50]])
                             .reshape(100, self.reshape[0], self.reshape[1], self.reshape[2]))
        image = img * 255
    ...
    def load_optimizer_weights(self, weights):
        self.adam_optimizer.set_weights(weights)
